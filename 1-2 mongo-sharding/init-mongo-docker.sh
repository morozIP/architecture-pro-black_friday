...
# -----------------------------------------------
# 1. Функция ожидания готовности сервиса (используя mongosh)
# -----------------------------------------------
wait_for_service() {
    SERVICE_HOST=$1
...
echo "============================================="
wait_for_service "$HOST_SHARD1"

cat <<EOF | $MONGO_SHELL --host "$HOST_SHARD1"
rs.initiate(
    {
      _id : "$SHARD1_RS_NAME",
      members: [
...
echo "============================================="
wait_for_service "$HOST_SHARD2"

cat <<EOF | $MONGO_SHELL --host "$HOST_SHARD2"
rs.initiate(
    {
      _id : "$SHARD2_RS_NAME",
      members: [
...
// Добавление шардов
sh.addShard( "$SHARD1_RS_NAME/$HOST_SHARD1");
sh.addShard( "$SHARD2_RS_NAME/$HOST_SHARD2");
